...
if [ -n "$FAISS_FLATTEN_CONDA_INCLUDES" ] && [ "$FAISS_FLATTEN_CONDA_INCLUDES" = "1" ]; then
  ...
  # cp -r -n "$CONDA_PREFIX/x86_64-conda-linux-gnu/include/c++/14.1.0/"* "$CONDA_PREFIX/include/"
  # cp -r -n "$CONDA_PREFIX/x86_64-conda-linux-gnu/include/c++/14.1.0/x86_64-conda-linux-gnu/"* "$CONDA_PREFIX/include/"
  ls "$CONDA_PREFIX/x86_64-conda-linux-gnu/include/c++" || :
fi
