...
# Uses libfaiss-spark-cu132 staged by build_lib_spark.sh
...
export LD_LIBRARY_PATH="$CUDA_HOME/lib64:${FAISS_ROOT}/_libfaiss_stage_spark/lib:$LD_LIBRARY_PATH"
...
if [ ! -d "_libfaiss_stage_spark" ]; then
    echo "ERROR: libfaiss-spark not staged. Run build_lib_spark.sh first."
    exit 1
fi
...
rm -rf "$BUILD_DIR"
cmake -B "$BUILD_DIR" \
    -Dfaiss_ROOT=_libfaiss_stage_spark/ \
    -DCMAKE_LIBRARY_PATH="${FAISS_ROOT}/_libfaiss_stage_spark/lib" \
    -DCMAKE_SHARED_LINKER_FLAGS="-L${FAISS_ROOT}/_libfaiss_stage_spark/lib" \
