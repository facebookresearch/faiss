...
`gpu-cu132/scripts/build_lib_cuda132.sh` requires Intel MKL (`mkl_rt`) and may need explicit paths when auto-detection does not match your shell environment.
...
```bash
bash gpu-cu132/scripts/build_wheel.sh
```

**Manual build:**
```bash
# Step 1: Build the C++ library
bash gpu-cu132/scripts/build_lib_cuda132.sh

# Step 2: Build Python bindings and wheel
bash gpu-cu132/scripts/build_pkg_cuda132.sh
```
...
bash gpu-cu132/scripts/clean_build.sh
