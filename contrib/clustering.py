...
def two_level_clustering(xt, nc1, nc2, clustering_niter=25, **args):
    ...
    nc1 and nc2 are the number of clusters at each level, the final number of clusters is nc1 * nc2
    Additional arguments are passed to the Kmeans object
    """
    d = xt.shape[1]
    ...
    for c1 in range(nc1):
        nc2 = int(all_nc2[c1])
        if verbose:
            print(f"[{time.time() - t0:.2f} s] training sub-cluster {c1}/{nc1} nc2={nc2}\r", end="", flush=True)
        i1 = i0 + bc[c1]
        subset = o[i0:i1]
...
#
# It relies on DatasetAssign, that offers the minimal set of operations to perform
# k-means clustering.
###############################################################################
...
def sparse_assign_to_dense_blocks(xq, xb, xq_norms=None, xb_norms=None, qbs=16384, bbs=16384, nt=None):
    """
    decomposes the sparse_assign_to_dense function into blocks to avoid a
    possible memory blow up. Can be run in multithreaded mode, because scipy's sparse-dense
    matrix multiplication is single-threaded.
    """
    nq = xq.shape[0]
    nb = xb.shape[0]
