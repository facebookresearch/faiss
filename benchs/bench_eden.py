...
stress distributions are available for evaluating behavior under heavier-tailed
or anisotropic data.
...
def print_markdown_tables(
    ...
    target = comparison_target(indexes)
    baselines = [name for name in indexes if name != target]
    ...
    for (distribution, rotation), rows in grouped.items():
        ...
        for index_name in indexes:
            header += f" {display_name(index_name)} recall |"
            align += "---:|"
            if include_ci:
                header += f" {display_name(index_name)} recall {ci_label} |"
                align += "---:|"
        if target is not None:
            for baseline in baselines:
                header += (
                    f" {display_name(target)} - {display_name(baseline)}"
                    " recall |"
                )
                align += "---:|"
                if include_ci and include_recall_delta_ci:
                    ...
                    align += "---:|"
        for index_name in indexes:
            header += f" {display_name(index_name)} MSE |"
            align += "---:|"
        if target is not None:
            for baseline in baselines:
                header += f" MSE reduction vs {display_name(baseline)} |"
                align += "---:|"
                if include_ci:
                    header += (
                        f" MSE reduction vs {display_name(baseline)}"
                        f" {ci_label} |"
                    )
                    align += "---:|"
        if include_speed:
            for index_name in indexes:
                header += f" {display_name(index_name)} search ms |"
                align += "---:|"
        ...
        for row in rows:
            ...
            if target is not None:
                for baseline in baselines:
                    mse_reduction = None
                    if target in row.metrics and baseline in row.metrics:
                        mse_reduction = (
                            row.metrics[baseline].mse - row.metrics[target].mse
                        )
...
def main():
    ...
    print(
        ...
        "SyntheticDataset is the default workload. Use EDENBiased for "
        "EDEN's MSE-minimizing scale."
    )
    print(
            ...
            for bits in args.bits:
                ...
                if target in metrics:
                    for baseline in args.indexes:
                        if baseline == target or baseline not in metrics:
                            continue
                        key = pair_key(target, baseline)
                        recall_delta = (
                            metrics[target].recall_by_query
                            - metrics[baseline].recall_by_query
                        )
                        mse_reduction = (
                            metrics[baseline].mse_by_vector
                            - metrics[target].mse_by_vector
                        )
