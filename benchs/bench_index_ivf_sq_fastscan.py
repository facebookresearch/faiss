...
def make_ivfsq(d, nlist, qtype, db, train):
...
def make_ivfsqfs(d, nlist, qtype, db, train, rerank_factor=4):
...
for qname, qtype in qtypes:
    ...
    try:
        index_sq = make_ivfsq(d, nlist, qtype, db, train)
    except Exception as e:
        print("%-30s %-22s  SKIP (%s)" % ("IndexIVFScalarQuantizer", qname, e))
        print()
        continue

    results_sq = evaluate_at_nprobes(index_sq, query, gt, k, nprobes)
    ...
    if qtype in rerank_types:
        ...
        for rf in rerank_factors:
            try:
                index_fs = make_ivfsqfs(d, nlist, qtype, db, train, rf)
            except Exception as e:
                print("%-30s %-22s  SKIP (%s)" % (
                    "IndexIVFSQFastScan(rf=%d)" % rf, qname, e))
                continue

            results_fs = evaluate_at_nprobes(index_fs, query, gt, k, nprobes)
        ...
        try:
            index_fs = make_ivfsqfs(d, nlist, qtype, db, train)
        except Exception as e:
            print("%-30s %-22s  SKIP (%s)" % (
