...
AutoTuneCriterion::AutoTuneCriterion(idx_t nq_, idx_t nnn_)
        : nq(nq_), nnn(nnn_), gt_nnn(0) {}
...
void AutoTuneCriterion::set_groundtruth(
    ...
    if (gt_D_in) { // allow null for this, as it is often not used
        gt_D.resize(nq * gt_nnn_);
        memcpy(gt_D.data(), gt_D_in, sizeof(gt_D[0]) * nq * gt_nnn_);
    }
    gt_I.resize(nq * gt_nnn_);
    memcpy(gt_I.data(), gt_I_in, sizeof(gt_I[0]) * nq * gt_nnn_);
}

OneRecallAtRCriterion::OneRecallAtRCriterion(idx_t nq_, idx_t R_)
        : AutoTuneCriterion(nq_, R_), R(R_) {}

double OneRecallAtRCriterion::evaluate(const float* /*D*/, const idx_t* I)
        const {
    FAISS_THROW_IF_NOT_MSG(
            (static_cast<idx_t>(gt_I.size()) == gt_nnn * nq && gt_nnn >= 1 && nnn >= R),
...
IntersectionCriterion::IntersectionCriterion(idx_t nq_, idx_t R_)
        : AutoTuneCriterion(nq_, R_), R(R_) {}

double IntersectionCriterion::evaluate(const float* /*D*/, const idx_t* I)
        const {
    FAISS_THROW_IF_NOT_MSG(
            (static_cast<idx_t>(gt_I.size()) == gt_nnn * nq && gt_nnn >= R && nnn >= R),
            "ground truth not initialized");
    int64_t n_ok = 0;
...
void ParameterSpace::explore(
    ...
    FAISS_THROW_IF_NOT_MSG(
            static_cast<idx_t>(nq) == crit.nq, "criterion does not have the same nb of queries");
...
#pragma omp parallel for
                for (idx_t q0 = 0; q0 < static_cast<idx_t>(nq); q0 += batchsize) {
