        ...
        for (int b = 0; b < BB; b++) {
            ...
            for (int q = 0; q < NQ; q++) {
                ...
                if (sq < nsq - nscale) {
                    ...
                    accu[q][b][0] += simd16uint16(res0) * scale_factor;
                    accu[q][b][1] += (simd16uint16(res0) >> 8) * scale_factor;

                    accu[q][b][2] += simd16uint16(res1) * scale_factor;
                    accu[q][b][3] += (simd16uint16(res1) >> 8) * scale_factor;
                }
            }
        }
    }
...
void accumulate_fixed_blocks(
    ...
    for (int64_t j0 = 0; j0 < nb; j0 += bbs) {
        ...
        kernel_accumulate_block<NQ, BB>(nsq, nscale, scale, codes, LUT, res2);
...
#define DISPATCH(NQ, BB)                                  \
    case NQ * 1000 + BB:                                  \
        accumulate_fixed_blocks<NQ, BB>(                  \
                nb, nsq, nscale, scale, codes, LUT, res); \
        break
...
#define INSTANTIATE_ACCUMULATE(TH, C, with_id_map)         \
    template void aq4_accumulate_loop<TH<C, with_id_map>>( \
            int,                                           \
            size_t,                                        \
            int,                                           \
            int,                                           \
