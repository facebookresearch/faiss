...
struct Top1BlockResultHandler : TopkBlockResultHandler<C, use_sel> {
    ...
    Top1BlockResultHandler(
            ...
            TI* ids_tab_,
            const IDSelector* sel_ = nullptr)
            : TopkBlockResultHandler<C, use_sel>(nq_, dis_tab_, ids_tab_, 1, sel_) {
    }
...
struct HeapBlockResultHandler : TopkBlockResultHandler<C, use_sel> {
    ...
    HeapBlockResultHandler(
            ...
            size_t k_,
            const IDSelector* sel_ = nullptr)
            : TopkBlockResultHandler<C, use_sel>(nq_, dis_tab_, ids_tab_, k_, sel_) {
    }
...
#pragma omp parallel for
        for (int64_t i = static_cast<int64_t>(i0); i < static_cast<int64_t>(i1); i++) {
            T* heap_dis = this->dis_tab + i * k;
            TI* heap_ids = this->ids_tab + i * k;
            const T* dis_tab_i = dis_in + (j1 - j0) * (i - static_cast<int64_t>(i0)) - j0;
            T thresh = heap_dis[0];
            for (size_t j = j0; j < j1; j++) {
                T dis = dis_tab_i[j];
                if (C::cmp(thresh, dis)) {
...
#pragma omp parallel for
        for (int64_t i = static_cast<int64_t>(i0); i < static_cast<int64_t>(i1); i++) {
            ReservoirTopN<C>& reservoir = reservoirs[i - static_cast<int64_t>(i0)];
            const T* dis_tab_i = dis_in + (j1 - j0) * (i - static_cast<int64_t>(i0)) - j0;
            for (size_t j = j0; j < j1; j++) {
                T dis = dis_tab_i[j];
