...
struct DCTemplate<Quantizer, Similarity, SIMDLevel::NONE> : SQDistanceComputer {
    ...
    float compute_distance(const float* x, const uint8_t* code) const {
        ...
        for (size_t i = 0; i < quant.d; i++) {
            float xi = quant.reconstruct_component(code, static_cast<int>(i));
    ...
    float compute_code_distance(const uint8_t* code1, const uint8_t* code2)
        ...
        sim.begin();
        for (size_t i = 0; i < quant.d; i++) {
            float x1 = quant.reconstruct_component(code1, static_cast<int>(i));
            float x2 = quant.reconstruct_component(code2, static_cast<int>(i));
