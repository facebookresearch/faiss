...
/*
 * Unified kernel: selects 256-bit vs 512-bit path based on SL.
 * Replaces the compile-time #ifndef __AVX512F__ guard with
 * a template-based selection.
 */
template <
        int NQ,
...
void kernel_accumulate_block(
    ...
    if constexpr (SL == SIMDLevel::AVX512 || SL == SIMDLevel::AVX512_SPR) {
        pq4_kernel_qbs_512<NQ, SL>(nsq, codes, LUT, res, scaler);
    } else {
...
void accumulate_q_4step(
    ...
    for (size_t j0 = 0; j0 < ntotal2; j0 += 32) {
        ...
        kernel_accumulate_block<Q1, SL>(nsq, codes, LUT, res2, scaler);
        LUT += Q1 * nsq * 16;
        if (Q2 > 0) {
            res2.set_block_origin(Q1, 0);
            kernel_accumulate_block<Q2, SL>(nsq, codes, LUT, res2, scaler);
            LUT += Q2 * nsq * 16;
        }
        if (Q3 > 0) {
            res2.set_block_origin(Q1 + Q2, 0);
            kernel_accumulate_block<Q3, SL>(nsq, codes, LUT, res2, scaler);
            LUT += Q3 * nsq * 16;
        }
        if (Q4 > 0) {
            res2.set_block_origin(Q1 + Q2 + Q3, 0);
            kernel_accumulate_block<Q4, SL>(nsq, codes, LUT, res2, scaler);
        }
...
void kernel_accumulate_block_loop(
    ...
    for (size_t j0 = 0; j0 < ntotal2; j0 += 32) {
        res.set_block_origin(0, j0);
        kernel_accumulate_block<NQ, SL>(nsq, codes, LUT, res, scaler);
        codes += block_stride;
    }
}

// non-template version of accumulate kernel -- dispatches dynamically on nq
...
#define DISPATCH(NQ)                                                  \
    case NQ:                                                          \
        kernel_accumulate_block_loop<NQ, SL>(                         \
