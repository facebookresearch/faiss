...
template <>
struct PQCodeDistance<PQDecoder8, SIMDLevel::AVX2> {
    static float distance_single_code(
        ...
        if (m < M) {
            // process leftovers
            PQDecoder8 decoder(code + m, static_cast<int>(nbits));
    ...
    static void distance_four_codes(
        ...
        if (m < M) {
            // process leftovers
            PQDecoder8 decoder0(code0 + m, static_cast<int>(nbits));
            PQDecoder8 decoder1(code1 + m, static_cast<int>(nbits));
            PQDecoder8 decoder2(code2 + m, static_cast<int>(nbits));
            PQDecoder8 decoder3(code3 + m, static_cast<int>(nbits));
            for (; m < M; m++) {
...
// template struct PQCodeDistance<PQDecoder8, SIMDLevel::AVX2>;

// these two will automatically use the generic implementation
template struct PQCodeDistance<PQDecoder16, SIMDLevel::AVX2>;
template struct PQCodeDistance<PQDecoderGeneric, SIMDLevel::AVX2>;
