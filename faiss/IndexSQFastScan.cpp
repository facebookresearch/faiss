...
void IndexSQFastScan::add(idx_t n, const float* x) {
    ...
    constexpr idx_t kAddBlock = 65536;
    if (n > kAddBlock) {
    ...
    if (needs_rerank(sq.qtype)) {
        ...
        const size_t base = lo_codes.size();
        lo_codes.resize(base + size_t(n) * stride, 0);
        const SQRanges ranges(sq, d);
        for (idx_t i = 0; i < n; i++) {
            const float* xi = x + i * d;
            uint8_t* dst = lo_codes.data() + base + size_t(i) * stride;
            BitstringWriter bsw(dst, stride);
            for (int m = 0; m < d; m++) {
                const int code = sq_code_of(
                        xi[m], ranges.vmin(m), ranges.vdiff(m), bits);
                bsw.write(code & ((1 << lo_bits) - 1), lo_bits);
            }
        }
    }
