        ...
        if (new_type == Array) {
            for (size_t ofs = 0; ofs < list_size; ofs++) {
                FAISS_THROW_IF_NOT_MSG(
                        0 <= idlist[ofs] && static_cast<size_t>(idlist[ofs]) < ntotal,
...
idx_t DirectMap::get(idx_t key) const {
    if (type == Array) {
        FAISS_THROW_IF_NOT_MSG(key >= 0 && static_cast<size_t>(key) < array.size(), "invalid key");
        idx_t lo = array[key];
...
void DirectMap::update_codes(
    ...
    for (int i = 0; i < n; i++) {
        idx_t id = ids[i];
        FAISS_THROW_IF_NOT_MSG(
                0 <= id && static_cast<size_t>(id) < array.size(), "id to update out of range");
        { // remove old one
            idx_t dm = array[id];
