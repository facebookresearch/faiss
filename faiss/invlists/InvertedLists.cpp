...
size_t InvertedLists::copy_subset_to(
    ...
    for (size_t list_no = 0; list_no < nlist; list_no++) {
        ...
        if (subset_type == SUBSET_TYPE_ID_RANGE) {
            ...
            if (static_cast<idx_t>(list_no) >= a1 && static_cast<idx_t>(list_no) < a2) {
                oivf.add_entries(
                        list_no,
                        n,
...
idx_t translate_list_no(const SliceInvertedLists* sil, idx_t list_no) {
    FAISS_THROW_IF_NOT(list_no >= 0 && static_cast<size_t>(list_no) < sil->nlist);
...
int translate_list_no(const VStackInvertedLists* vil, idx_t list_no) {
    FAISS_THROW_IF_NOT(list_no >= 0 && static_cast<size_t>(list_no) < vil->nlist);
