...
struct MinSumK {
    ...
    MinSumK(int K_in, int M_in, int nbit_in, int N_in) : K(K_in), M(M_in), nbit(nbit_in), N(N_in) {
...
MultiIndexQuantizer2::MultiIndexQuantizer2(
    ...
    FAISS_THROW_IF_NOT_MSG(
            static_cast<size_t>(assign_index_0->d) == pq.dsub && static_cast<size_t>(assign_index_1->d) == pq.dsub,
            "Provided sub-index has incorrect size");
