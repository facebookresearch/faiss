...
IndexIVF *fix_ivf_fields(IndexIVF *index_ivf) {
    index_ivf->quantizer_trains_alone =
            get_trains_alone(index_ivf->quantizer);
...
AdditiveQuantizer::Search_type_t parse_AQ_search_type(std::string stok, MetricType metric) {
    ...
    for (char* tok = strtok_r(&description[0], " ,", &ptr); tok;
        ...
        if (sscanf(tok, "PCA%d", &d_out) == 1) {
            ...
            use_2layer = true;
                ...
                stok,
                std::regex("(RQ|RCQ)[0-9]+x[0-9]+(_[0-9]+x[0-9]+)*(_Nnone|_Nfloat|_Nqint8|_Nqint4)?"))
            ) {
            ...
            if (is_RCQ) {
                ...
                if (!coarse_quantizer) {
                    index_1 = new IndexResidual(d, nbits, metric, st);
                } else {
                    index_1 = fix_ivf_fields(new IndexIVFResidual(
                        coarse_quantizer, d, ncentroids,
                        nbits, metric, st
                    ));
                ...
                stok,
                std::regex("(LSQ|LSCQ)[0-9]+x[0-9]+(_Nnone|_Nfloat|_Nqint8|_Nqint4)?"))
            ) {
            std::vector<size_t> nbits;
            std::smatch sm;
            ...
            if (is_LSCQ) {
                ...
                AdditiveQuantizer::Search_type_t st =
                    parse_AQ_search_type(stok, metric);
                if (!coarse_quantizer) {
                    index_1 = new IndexLocalSearchQuantizer(d, M, nbit, metric, st);
                } else {
