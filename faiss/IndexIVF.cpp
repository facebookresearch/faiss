...
void Level1Quantizer::train_q1(
    ...
    if (quantizer->is_trained && (static_cast<size_t>(quantizer->ntotal) == nlist)) {
        if (verbose) {
...
IndexIVF::IndexIVF(
        ...
        : Index(d_in, metric),
          ...
          invlists(
                  own_invlists_in ? new ArrayInvertedLists(nlist_in, code_size_in)
                                  : nullptr),
          own_invlists(own_invlists_in),
          code_size(code_size_in) {
    FAISS_THROW_IF_NOT(static_cast<int>(d_in) == quantizer_in->d);
    is_trained = quantizer_in->is_trained && (static_cast<size_t>(quantizer_in->ntotal) == nlist_in);
...
void IndexIVF::search_preassigned(
    ...
    FAISS_THROW_IF_NOT_MSG(
            !invlists->use_iterator || (cur_max_codes == 0 && store_pairs == false),
...
void IndexIVF::update_vectors(int n, const idx_t* new_ids, const float* x) {
    if (direct_map.type == DirectMap::Hashtable) {
        ...
        FAISS_THROW_IF_NOT_MSG(
                nremove == static_cast<size_t>(n), "did not find all entries to remove");
