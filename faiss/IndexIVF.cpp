...
void IndexIVF::boundary_search_preassigned(
        idx_t n,
        const float* x,
        idx_t k,
        const float lower,
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap)
    {
        ...
        // single list scan using the current scanner (with query
        // set porperly) and storing results in simi and idxi
        auto scan_one_list = [&](const float lower,
                                const float upper,
                                idx_t key,
                                 float coarse_dis_i,
                                 float* simi,
...
size_t InvertedListScanner::scan_codes_boundary(
        size_t list_size,
        const float lower,
        const float upper,
        const uint8_t* codes,
...
size_t InvertedListScanner::iterate_codes_boundary(
        InvertedListsIterator* it,
        const float lower,
        const float upper,
        float* simi,
