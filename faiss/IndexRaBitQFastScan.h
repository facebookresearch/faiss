...
struct IndexRaBitQFastScan : IndexFastScan {
    ...
    /// Mutable because compute_codes needs to resize this during encoding
    mutable std::vector<FactorsData> factors_storage;

    /// Query factors storage for reconstruction during search
    /// Size: n_queries, stores query-specific factors for distance computation
    /// Mutable because search needs to resize this during query processing
    mutable std::vector<QueryFactorsData> query_factors_storage;
    ...
    void compute_float_LUT(
            ...
            idx_t n,
            const float* x,
            idx_t query_offset = 0) const override;
...
struct RaBitQHeapHandler : SIMDResultHandler {
    ...
    void set_block_origin(size_t, size_t j0) final;
