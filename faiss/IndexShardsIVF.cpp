...
IndexShardsIVF::IndexShardsIVF(
    ...
    is_trained = quantizer_->is_trained && quantizer_->ntotal == static_cast<idx_t>(nlist_);
}
...
void IndexShardsIVF::add_with_ids(
    ...
    auto fn = [n, ids, x, nshard, cur_d, Iq](int no, Index* index) {
        ...
        index_ivf->add_core(
                i1 - i0, x + i0 * cur_d, ids ? ids + i0 : nullptr, Iq.data() + i0);
...
void IndexShardsIVF::search(
    ...
    auto fn = [&](int no, const Index* indexIn) {
        ...
        FAISS_THROW_IF_NOT_MSG(index->nprobe == static_cast<size_t>(nprobe), "inconsistent nprobe");
