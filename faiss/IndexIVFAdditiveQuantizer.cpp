...
struct AQInvertedListScannerDecompress : AQInvertedListScanner {
    ...
    float distance_to_code(const uint8_t* code) const final {
        ...
        return is_IP ?
            coarse_dis + fvec_inner_product(q, b.data(), aq.d)
...
struct AQInvertedListScannerLUT : AQInvertedListScanner {
    ...
    /// compute a single query-to-code distance
    float distance_to_code(const uint8_t* code) const final {
        return distance_bias + aq.compute_1_distance_LUT<is_IP, search_type>(code, LUT.data());
    }
...
IndexIVFResidual::IndexIVFResidual(
        Index* quantizer,
        size_t d,
        size_t nlist,
