...
IndexRaBitQFastScan::IndexRaBitQFastScan(
    ...
    init_fastscan(static_cast<int>(d_in), M_fastscan, nbits_fastscan, metric, bbs_in);
...
void IndexRaBitQFastScan::compute_codes(uint8_t* out_codes, idx_t n, const float* x)
        const {
            ...
            for (size_t m = 0; m < M; m++) {
                ...
                for (int code_val = 0; code_val < 16; code_val++) {
                    ...
                    for (size_t dim_offset = 0; dim_offset < 4; dim_offset++) {
                        ...
                        if (dim_idx < static_cast<size_t>(d) && ((code_val >> dim_offset) & 1)) {
...
void RaBitQHeapHandler<C, with_id_map>::handle(
    ...
    const size_t max_vectors = (base_db_idx < static_cast<size_t>(rabitq_index->ntotal))
            ? std::min<size_t>(32, static_cast<size_t>(rabitq_index->ntotal) - base_db_idx)
            : 0;
