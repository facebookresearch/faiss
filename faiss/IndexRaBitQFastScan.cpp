...
size_t IndexRaBitQFastScan::get_block_stride() const {
...
void IndexRaBitQFastScan::populate_block_aux_from_flat_storage(
        const std::vector<uint8_t>& legacy_flat_storage) {
    if (legacy_flat_storage.empty() || ntotal == 0) {
        return;
    }

    const size_t storage_size = compute_per_vector_storage_size();
    const size_t packed_block_size = ((M2 + 1) / 2) * bbs;
    const size_t old_block_size = packed_block_size;
    const size_t new_block_size = get_block_stride();

    FAISS_THROW_IF_NOT_MSG(
            legacy_flat_storage.size() ==
                    static_cast<size_t>(ntotal) * storage_size,
            "legacy flat_storage size mismatch during migration");

    const size_t n_blocks = ntotal2 / bbs;

    // Old format had smaller blocks (no aux region).
    // Re-layout: copy packed codes from old blocks into new enlarged blocks.
    AlignedTable<uint8_t> old_data;
    old_data.resize(codes.size());
    memcpy(old_data.data(), codes.data(), codes.size());

    codes.resize(n_blocks * new_block_size);
    memset(codes.get(), 0, n_blocks * new_block_size);

    for (size_t b = 0; b < n_blocks; b++) {
        memcpy(codes.get() + b * new_block_size,
    ...
    for (idx_t i = 0; i < ntotal; i++) {
        const uint8_t* src = legacy_flat_storage.data() + i * storage_size;
