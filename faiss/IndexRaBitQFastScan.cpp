...
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < n; i++) {
        ...
        // Use shared utilities for computing intermediate values
        float norm_L2sqr, or_L2sqr, dp_oO;
        rabitq_utils::compute_vector_intermediate_values(
                x_row, d, centroid_data, norm_L2sqr, or_L2sqr, dp_oO);

        // Compute factors using shared utilities
        fac = rabitq_utils::compute_factors_from_intermediates(
...
void IndexRaBitQFastScan::compute_float_LUT(
    ...
    for (idx_t i = 0; i < n; i++) {
        ...
        // Use shared utilities for query factor computation
...
void* IndexRaBitQFastScan::make_knn_handler(
    ...
    if (is_max) {
        return new RaBitQHeapHandler<CMax<uint16_t, int>, false>(
                this, n, k, distances, labels, query_offset, sel);
    } else {
