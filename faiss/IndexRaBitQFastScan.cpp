...
#include <faiss/impl/QueryFactorsProcessor.h>
...
void IndexRaBitQFastScan::add(idx_t n, const float* x) {
    ...
    // Resize factors storage to accommodate new vectors
    const idx_t start_idx = ntotal;
    factors_storage.resize(start_idx + n);

    // Compute factors for all new vectors before calling parent add()
    const float* centroid_data = center.data();
...
#pragma omp parallel for if (n > 1000)
    ...
    // Call parent implementation which will call compute_codes()
    // compute_codes() now only needs to compute bit patterns
...
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < n; i++) {
        ...
        // Factors are already computed in add() method
        for (size_t j = 0; j < d; j++) {
...
void IndexRaBitQFastScan::compute_float_LUT(
    ...
    // Use query factors from context if available
    QueryFactorsProcessor* query_processor = context.query_factors;
    ...
    for (idx_t i = 0; i < n; i++) {
        ...
        // Compute query factors and store in processor if available
        ...
        // Store query factors in processor if provided
        if (query_processor) {
            // Use specialized type-safe method
            query_processor->set_query_factors(
                    query_offset + i, query_factors_data);
        }
...
void IndexRaBitQFastScan::search(
    ...
    // Create QueryFactorsProcessor on stack with space allocated for query
    // factors Query factors will be computed in compute_float_LUT
    QueryFactorsProcessor query_processor(n);

    // Use the faster search_dispatch_implem flow from IndexFastScan
    // Pass the QueryFactorsProcessor - factors will be computed during LUT
    // computation
    ProcessingContext context;
    context.query_factors = &query_processor;
...
RaBitQHeapHandler<C, with_id_map>::RaBitQHeapHandler(
        ...
        QueryFactorsProcessor* query_factors)
...
void RaBitQHeapHandler<C, with_id_map>::handle(
    ...
    // Access query factors from query_factors processor
    QueryFactorsData query_factors_data = {};
    if (query_factors) {
        // Use specialized type-safe method
        query_factors_data =
                query_factors->get_query_factors(q + global_query_offset);
    }
