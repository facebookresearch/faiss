...
/*********************************************************************
 * Templated PQ code distance dispatch (any decoder type).
 *
 * Recommended API for callers that are templated on decoder type.
 * For PQDecoder8, dispatches to the best available SIMD level (same
 * as the non-templated overloads above). For other decoders, uses
 * scalar — their sequential decode() methods don't benefit from SIMD.
 *********************************************************************/

/// Compute PQ distance for a single code (decoder-generic).
template <typename PQDecoderT>
float pq_code_distance_single(
        ...
        const uint8_t* code) {
    if constexpr (std::is_same_v<PQDecoderT, PQDecoder8>) {
        return pq_code_distance_single(M, nbits, sim_table, code);
    } else {
        return PQCodeDistanceScalar<PQDecoderT>::distance_single_code(
                M, nbits, sim_table, code);
    }
...
/// Compute PQ distances for four codes simultaneously (decoder-generic).
template <typename PQDecoderT>
