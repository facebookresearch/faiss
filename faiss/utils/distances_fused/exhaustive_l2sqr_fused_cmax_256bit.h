...
#ifdef __x86_64__
#include <xmmintrin.h> // for _mm_prefetch, _MM_HINT_NTA
#endif
...
void kernel(
    ...
    using simd8float32 = simd8float32<simd_level>;
    using simd8uint32 = simd8uint32<simd_level>;
...
#define DISPATCH_L2SQR_FUSED_CMAX(                                    \
        FUNC, DIM, NX_POINTS_PER_LOOP, NY_POINTS_PER_LOOP, SIMDLevel) \
    case DIM: {                                                       \
        FUNC<DIM, NX_POINTS_PER_LOOP, NY_POINTS_PER_LOOP, SIMDLevel>( \
                x, y, nx, ny, res, y_norms);                          \
        return true;                                                  \
    }
...
template <
        ...
        size_t NY_POINTS_PER_LOOP,
        SIMDLevel simd_level>
void exhaustive_L2sqr_fused_cmax_core(
        const float* const __restrict x,
        const float* const __restrict y,
        size_t nx,
        size_t ny,
...
#pragma omp parallel for schedule(dynamic)
    for (int64_t i = 0; i < nx_p; i += NX_POINTS_PER_LOOP) {
        kernel<DIM, NX_POINTS_PER_LOOP, NY_POINTS_PER_LOOP, simd_level>(
                x, y, y_transposed.data(), ny, res, y_norms, i);
    }

    for (size_t i = nx_p; i < nx; i++) {
        kernel<DIM, 1, NY_POINTS_PER_LOOP, simd_level>(
                x, y, y_transposed.data(), ny, res, y_norms, i);
    }
