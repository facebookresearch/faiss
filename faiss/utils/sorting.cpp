...
void bucket_sort_inplace_ref(
    ...
    FAISS_THROW_IF_NOT(
            static_cast<size_t>(nbucket) < nval); // unclear what would happen in this case...
    ...
    for (;;) {
        ...
        if (next_bucket_no != -1) {
            ...
            for (; init_bucket_no < nbucket; init_bucket_no++) {
                if (ptrs[init_bucket_no] < static_cast<size_t>(lims[init_bucket_no + 1])) {
                    break;
                }
            }
...
#pragma omp master
            {
                ...
                if (verbose > 2) {
                    for (size_t b = 0; b < static_cast<size_t>(nbucket); b++) {
                        printf("   b=%zd [", b);
                        for (size_t i = static_cast<size_t>(lims[b]); i < static_cast<size_t>(lims[b + 1]); i++) {
                            printf(" %s%d",
                                   ptrs[b] == i ? ">" : "",
                                   int(vals[i]));
                        }
...
#pragma omp parallel for reduction(+ : num_errors)
    for (int64_t bucket = 0; bucket < static_cast<int64_t>(nbucket); bucket++) {
        ...
        for (size_t i = static_cast<size_t>(lims[bucket]); i < static_cast<size_t>(lims[bucket + 1]); i++) {
            int64_t j = perm[i];
            ...
            for (;;) {
                ...
                if (slot == static_cast<size_t>(hk[j])) { // no free slot left in bucket
                    num_errors++;
                    break;
                }
            }
    ...
    for (int64_t i = 0; i < static_cast<int64_t>(n); i++) {
        ...
        if (tab[2 * slot] == -1) { // not in table
            ...
            for (;;) {
                ...
                if (slot == static_cast<size_t>(hk_i)) { // bucket is full and not found
                    vals[i] = -1;
                    break;
                }
            }
