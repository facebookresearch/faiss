...
// AVX512_SPR has every feature AVX512_VPOPCNT needs, so it reuses these
// kernels. Without this, SINGLE_SIMD_LEVEL in a static SPR build names a
// level with no Hamming computer at all.
#define FAISS_INHERIT_HAMMING_SPR(Class)                           \
    template <>                                                    \
    struct Class##_tpl<SIMDLevel::AVX512_SPR>                      \
            : Class##_tpl<SIMDLevel::AVX512_VPOPCNT> {             \
        using Class##_tpl<SIMDLevel::AVX512_VPOPCNT>::Class##_tpl; \
    }

FAISS_INHERIT_HAMMING_SPR(HammingComputer16);
FAISS_INHERIT_HAMMING_SPR(HammingComputer20);
FAISS_INHERIT_HAMMING_SPR(HammingComputer32);
FAISS_INHERIT_HAMMING_SPR(HammingComputer64);
FAISS_INHERIT_HAMMING_SPR(HammingComputerDefault);
