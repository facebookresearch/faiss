...
QINCoStep::QINCoStep(int d_in, int K_in, int L_in, int h_in)
        : d(d_in), K(K_in), L(L_in), h(h_in), codebook(K_in, d_in), MLPconcat(2 * d_in, d_in) {
    for (int i = 0; i < L; i++) {
...
nn::Int32Tensor2D QINCoStep::encode(
    ...
    size_t n = xhat.shape[0];
    FAISS_THROW_IF_NOT(
            n == x.shape[0] && xhat.shape[1] == static_cast<size_t>(d) && x.shape[1] == static_cast<size_t>(d));
    ...
    if (residuals) {
        FAISS_THROW_IF_NOT(
                residuals->shape[0] == n && residuals->shape[1] == static_cast<size_t>(d));
        res = residuals->data();
    }
...
QINCo::QINCo(int d_in, int K_in, int L_in, int M_in, int h_in)
        : NeuralNetCodec(d_in, M_in), K(K_in), L(L_in), h(h_in), codebook0(K_in, d_in) {
    for (int i = 1; i < M; i++) {
