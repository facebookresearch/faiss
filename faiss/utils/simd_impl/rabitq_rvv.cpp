...
uint64_t bitwise_and_dot_product<SIMDLevel::RISCV_RVV>(
    ...
    while (i < size) {
        ...
        for (size_t j = 0; j < qb; j++) {
            ...
            vuint8m4_t vp = __riscv_vand_vv_u8m4(vx, vq, vl);
            vq = __riscv_vsrl_vx_u8m4(vp, 1, vl);
            vq = __riscv_vand_vx_u8m4(vq, 0x55, vl);
            vp = __riscv_vsub_vv_u8m4(vp, vq, vl);
            vq = __riscv_vsrl_vx_u8m4(vp, 2, vl);
            vq = __riscv_vand_vx_u8m4(vq, 0x33, vl);
            vp = __riscv_vand_vx_u8m4(vp, 0x33, vl);
            vp = __riscv_vadd_vv_u8m4(vp, vq, vl);
            vq = __riscv_vsrl_vx_u8m4(vp, 4, vl);
            vp = __riscv_vadd_vv_u8m4(vp, vq, vl);
            vp = __riscv_vand_vx_u8m4(vp, 0x0F, vl);
...
uint64_t bitwise_xor_dot_product<SIMDLevel::RISCV_RVV>(
    ...
    while (i < size) {
        ...
        for (size_t j = 0; j < qb; j++) {
            vuint8m4_t vq = __riscv_vle8_v_u8m4(query + j * size + i, vl);
            vuint8m4_t vp = __riscv_vxor_vv_u8m4(vx, vq, vl);
            vq = __riscv_vsrl_vx_u8m4(vp, 1, vl);
            vq = __riscv_vand_vx_u8m4(vq, 0x55, vl);
            vp = __riscv_vsub_vv_u8m4(vp, vq, vl);
            vq = __riscv_vsrl_vx_u8m4(vp, 2, vl);
...
uint64_t popcount<SIMDLevel::RISCV_RVV>(const uint8_t* data, size_t size) {
    ...
    while (i < size) {
        size_t vl = __riscv_vsetvl_e8m4(size - i);
        vuint8m4_t v = __riscv_vle8_v_u8m4(data + i, vl);
