...
void IndexIVFSQFastScan::search(
    ...
    if (is_fallback(sq.qtype)) {
        ...
        IndexIVFFastScan::search(
                n, x, k_coarse, coarse_dis.data(), coarse_ids.data(), params);

        // Rerank using original full-precision codes
#pragma omp parallel
        {
...
#pragma omp for
            for (idx_t i = 0; i < n; i++) {
                ...
                idx_t* heap_ids = labels + i * k;

                if (metric_type == METRIC_L2) {
                    maxheap_heapify(k, heap_dis, heap_ids);
                } else {
                    minheap_heapify(k, heap_dis, heap_ids);
                }
                ...
                for (idx_t j = 0; j < k_coarse; j++) {
                    ...
                    if (metric_type == METRIC_L2) {
                        if (dis < heap_dis[0]) {
                            maxheap_replace_top(k, heap_dis, heap_ids, dis, id);
                        }
                    } else {
                        if (dis > heap_dis[0]) {
                            minheap_replace_top(k, heap_dis, heap_ids, dis, id);
                        }
                    }
                }
                ...
                if (metric_type == METRIC_L2) {
                    ...
                    minheap_reorder(k, heap_dis, heap_ids);
                }
            }
        }
    } else {
...
void IndexIVFSQFastScan::range_search(
    ...
    if (is_fallback(sq.qtype)) {
        ...
        size_t cur_nprobe = ivf_params ? ivf_params->nprobe : nprobe;
        const IDSelector* sel = params ? params->sel : nullptr;

        std::vector<idx_t> assign(n * cur_nprobe);
        std::vector<float> centroid_dis(n * cur_nprobe);
        quantizer->search(n, x, cur_nprobe, centroid_dis.data(), assign.data());
...
#pragma omp for
            for (idx_t i = 0; i < n; i++) {
                ...
                for (size_t p = 0; p < cur_nprobe; p++) {
                    idx_t list_no = assign[i * cur_nprobe + p];
                    if (list_no < 0) {
                        continue;
                    }
