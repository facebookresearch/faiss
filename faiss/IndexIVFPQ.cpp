...
struct IVFPQScannerT : QueryTables {
    ...
    IVFPQScannerT(const IndexIVFPQ& ivfpq_in, const IVFSearchParameters* params_in)
    ...
    void scan_on_the_fly_dist(
        ...
        for (size_t j = 0; j < ncode; j++, codes += pq.code_size) {
            ...
            float dis;
            if (METRIC_TYPE == METRIC_INNER_PRODUCT) {
                dis = local_dis0 + fvec_inner_product_dispatch(decoded_vec, qi, d);
            } else {
...
struct IVFPQScanner : IVFPQScannerT<idx_t, METRIC_TYPE, PQDecoder>,
    ...
    IVFPQScanner(
            ...
            const IDSelector* sel_in)
            : IVFPQScannerT<idx_t, METRIC_TYPE, PQDecoder>(ivfpq_in, nullptr),
