...
void IndexIVFRaBitQFastScan::search(
    ...
    // Create QueryFactorsProcessor and ProcessingContext
    QueryFactorsProcessor query_processor(n * nlist);
...
#pragma omp parallel for if (n * nprobe > 1000)
    for (idx_t ij = 0; ij < n * nprobe; ij++) {
        ...
        if (cij >= 0) {
            ...
            // Store in QueryFactorsProcessor
            if (context.query_factors &&
                context.query_factors->has_query_factors()) {
                context.query_factors->set_query_factors(
                        storage_idx, query_factors_data);
            }
...
SIMDResultHandlerToFloat* IndexIVFRaBitQFastScan::make_knn_handler(
    ...
    if (is_max) {
        return new IVFRaBitQHeapHandler<CMax<uint16_t, int64_t>>(
                this, n, k, distances, labels, query_offset, &context);
    } else {
...
void IndexIVFRaBitQFastScan::IVFRaBitQHeapHandler<C>::handle(
    ...
    // Access query factors through ProcessingContext if available
    const QueryFactorsData* query_factors_ptr = nullptr;

    if (context && context->query_factors &&
        context->query_factors->has_query_factors()) {
        // Use new QueryFactorsProcessor
        query_factors_ptr =
                &context->query_factors->get_query_factors(storage_idx);
    } else {
        FAISS_THROW_MSG(
                "Query factors not available: ProcessingContext with QueryFactorsProcessor required");
    }

    const auto& query_factors = *query_factors_ptr;
