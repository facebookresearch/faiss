...
struct IndexIVFRaBitQFastScan : IndexIVFFastScan {
   ...
   public:
    /// Return enlarged block stride for SIMD accumulation
    size_t get_block_stride() const override;
    ...
    /// Migrate block auxiliary data after deserialization of old-format
    /// indexes. Old-format indexes (fourcc "Iwrf") stored per-vector factors in
    /// a separate flat_storage vector. This method re-layouts blocks to the
    /// enlarged format and copies aux data from the provided flat_storage.
