...
void sa_decode_impl(
    ...
    std::vector<uint8_t> tmp(
            (chunk_size < static_cast<size_t>(n_input) ? chunk_size : static_cast<size_t>(n_input)) * old_code_size);
    std::vector<StorageMinMaxFP16> minmax(
            (chunk_size < static_cast<size_t>(n_input) ? chunk_size : static_cast<size_t>(n_input)));
