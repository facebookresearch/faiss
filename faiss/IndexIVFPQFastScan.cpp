...
struct IVFPQFastScanScanner : InvertedListScanner {
    static const int impl = 10; // based on search_implem_10
    static const size_t nq = 1; // 1 query at a time.
    ...
    void set_query(const float* query) override {
        ...
        if (this->index.by_residual) {
            cqwb.quantize(index.quantizer, nq, xi, nullptr);
            index.invlists->prefetch_lists(cqwb.ids, nq * cqwb.nprobe);
        }
        // Handler handles results of 1 inverted list search.
        index.compute_LUT_uint8(
                1, xi, cqwb, dis_tables, biases, &normalizers[0]);
    }
    ...
    size_t scan_codes(
        ...
        handler->q_map = qmap1;
        handler->begin(index.skip & 16 ? nullptr : &normalizers[0]);
