...
IndexBinaryIVF::IndexBinaryIVF(
    ...
    is_trained = quantizer_->is_trained && (static_cast<size_t>(quantizer_->ntotal) == nlist_);
    cp.niter = 10;
}
...
void IndexBinaryIVF::replace_invlists(InvertedLists* il, bool own) {
    FAISS_THROW_IF_NOT(il->nlist == nlist && il->code_size == static_cast<size_t>(code_size));
    if (own_invlists) {
        delete invlists;
    }
    invlists = il;
