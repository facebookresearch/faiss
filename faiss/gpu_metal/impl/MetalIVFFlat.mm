...
constexpr size_t kMaxU32SizeT = (size_t)std::numeric_limits<uint32_t>::max();
...
void MetalIVFFlatImpl::uploadToGpu_(
    ...
    if (metaBytes > 0) {
        if (listOffsetBuf_ == nil || [listOffsetBuf_ length] < metaBytes) {
            if (listOffsetBuf_ != nil) {
                resources_->deallocBuffer(
                        listOffsetBuf_, MetalAllocType::IVFLists);
            }
            listOffsetBuf_ = resources_->allocBuffer(
                    metaBytes, MetalAllocType::IVFLists);
        }
        if (listLengthBuf_ == nil || [listLengthBuf_ length] < metaBytes) {
            if (listLengthBuf_ != nil) {
                resources_->deallocBuffer(
                        listLengthBuf_, MetalAllocType::IVFLists);
            }
            listLengthBuf_ = resources_->allocBuffer(
                    metaBytes, MetalAllocType::IVFLists);
        }
        FAISS_THROW_IF_NOT_MSG(
    ...
    if (idsBuffer_ == nil || [idsBuffer_ length] < idsBytes) {
        ...
        idsBuffer_ =
                resources_->allocBuffer(idsBytes, MetalAllocType::IVFLists);
        forceFullUpload = true;
    }

    FAISS_THROW_IF_NOT_MSG(
            idsBuffer_, "MetalIVFFlatImpl: failed to allocate IVF ids buffer");
    ...
    if (forceFullUpload) {
        ...
        for (size_t l = 0; l < (size_t)nlist_; ++l) {
            ...
            size_t start =
                    moved ? listOffset_[l] : (listOffset_[l] + oldLength[l]);
...
void MetalIVFFlatImpl::rebuildInterleavedBuffers_() {
    ...
    auto* ptr =
            reinterpret_cast<uint32_t*>([interleavedCodesOffsetBuf_ contents]);
    for (size_t i = 0; i < (size_t)nlist_; ++i) {
        ptr[i] = ilOffsets[i];
    }
    interleavedDirty_ = false;
...
void MetalIVFFlatImpl::ensureInterleavedLayoutUpToDate() {
    ...
    if (!interleavedDirty_ && interleavedCodesBuf_ &&
        interleavedCodesOffsetBuf_) {
        return;
    }
