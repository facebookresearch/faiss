...
TEST_F(TestMetalIndexIVFFlat, L2_TrainAddSearch) {
    ...
    faiss::gpu_metal::MetalIndexIVFFlat metalIndex(resources_, &cpuIndex);
    ...
    metalIndex.search(nq, queries.data(), k, testDist.data(), testLab.data());
...
TEST_F(TestMetalIndexIVFFlat, CpuToGpuRoundTrip) {
    ...
    std::unique_ptr<faiss::Index> cpuBack(
            faiss::gpu_metal::index_metal_gpu_to_cpu(metalIndex.get()));
    ...
    cpuBack->search(nq, queries.data(), k, testDist.data(), testLab.data());
