...
void IndexFastScan::compute_quantized_LUT(
    ...
    compute_float_LUT(dis_tables.get(), n, x, query_offset, context);
...
void IndexFastScan::search(
    ...
    ProcessingContext empty_context{};
...
void IndexFastScan::search_dispatch_implem(
    ...
    if (implem == 1) {
        ...
        // query_offset = 0: single thread processes all queries starting from
        // query 0
        if (nt < 2) {
            if (impl == 12 || impl == 13) {
                search_implem_12<C>(
                        n, x, k, distances, labels, impl, context, 0);
            } else {
                search_implem_14<C>(
                        n, x, k, distances, labels, impl, context, 0);
            }
...
void IndexFastScan::search_implem_234(
    ...
    std::unique_ptr<float[]> dis_tables(new float[n * dim12]);
    compute_float_LUT(dis_tables.get(), n, x, 0, context);
