...
DEFINE_string(in, "/root/local/index.out", "index file for input");
...
int main(int argc, char** argv) {
    ...
    auto index =
            std::unique_ptr<faiss::IndexIVFAQ>(dynamic_cast<faiss::IndexIVFAQ*>(
    ...
    auto dim = index->d;
    auto codes = index->aq.M;
    auto bitsPerCode = index->aq.nbits;

    printf("Database: dim %d num vecs %ld\n", dim, index->ntotal);
    printf("Coarse centroids: %ld\n", index->quantizer->ntotal);
    printf("AQ centroids: codes %ld bits per code %ld\n", codes, bitsPerCode);
    ...
    auto initFn =
            [precomp, indicesOpt, useFloat16Lookup, &index](
                    ...
                    int dev) -> std::unique_ptr<faiss::gpu::GpuIndexIVFAQ> {
        ...
        auto p = std::unique_ptr<faiss::gpu::GpuIndexIVFAQ>(
                new faiss::gpu::GpuIndexIVFAQ(res, index.get(), config));

        return p;
    };

    IndexWrapper<faiss::gpu::GpuIndexIVFAQ> gpuIndex(FLAGS_num_gpus, initFn);
