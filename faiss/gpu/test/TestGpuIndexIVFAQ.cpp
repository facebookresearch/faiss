...
struct Options {
    Options() {
        ...
        //   support non-multiple of 8 subcodes for IVFAQ.
        bitsPerCode = 8;
    ...
    std::string toString() const {
        ...
        str << "IVFAQ device " << device << " numVecs " << numAdd << " dim "
...
TEST(TestGpuIndexIVFAQ, Query_L2) {
...
void testMMCodeDistance(faiss::MetricType mt) {
    ...
    // delete it. AQ don't have sub-quantizer
}

TEST(TestGpuIndexIVFAQ, Query_L2_MMCodeDistance) {
    testMMCodeDistance(faiss::MetricType::METRIC_L2);
}

TEST(TestGpuIndexIVFAQ, Query_IP_MMCodeDistance) {
    testMMCodeDistance(faiss::MetricType::METRIC_INNER_PRODUCT);
}

TEST(TestGpuIndexIVFAQ, Query_IP) {
    for (int tries = 0; tries < 2; ++tries) {
        Options opt;
...
TEST(TestGpuIndexIVFAQ, Float16Coarse) {
    Options opt;
...
TEST(TestGpuIndexIVFAQ, CopyTo) {
    for (int tries = 0; tries < 2; ++tries) {
        ...
        faiss::gpu::GpuIndexIVFAQ gpuIndex(
                &res,
                opt.dim,
                opt.numCentroids,
                opt.codes,
        ...
        faiss::IndexIVFAQ cpuIndex(&cpuQuantizer, 1, 1, 1, 1);
        ...
        EXPECT_EQ(cpuIndex.aq.M, gpuIndex.getNumSubQuantizers());
        EXPECT_EQ(gpuIndex.getNumSubQuantizers(), opt.codes);
        EXPECT_EQ(cpuIndex.aq.nbits, gpuIndex.getBitsPerCode());
...
TEST(TestGpuIndexIVFAQ, CopyFrom) {
    Options opt;
    std::vector<float> trainVecs = faiss::gpu::randVecs(opt.numTrain, opt.dim);
    std::vector<float> addVecs = faiss::gpu::randVecs(opt.numAdd, opt.dim);
...
TEST(TestGpuIndexIVFAQ, QueryNaN) {
    Options opt;
...
TEST(TestGpuIndexIVFAQ, UnifiedMemory) {
    ...
    // Unfortunately it would take forever to add 24 GB in IVFAQ data,
