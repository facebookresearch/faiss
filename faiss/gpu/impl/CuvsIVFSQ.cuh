...
class CuvsIVFSQ : public IVFFlat {
   ...
   private:
    ...
    /// Largest vector id currently stored, tracked incrementally at add time so
    /// IDSelector filtering can size its bitset without rescanning the index.
    /// -1 means no vectors are stored.
    idx_t maxVectorId_{-1};
