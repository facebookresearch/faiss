...
/// Implementing class for IMIPQ on the GPU
class IMIPQ : public IMIBase {
   public:
    IMIPQ(GpuResources* resources,
    ...
    /// Find the approximate k nearest neigbors for
    /// `queries` (n) (dim) against our database
    void query_split(
            Tensor<float, 2, true>& queries,
            int nprobe,
            int k,
