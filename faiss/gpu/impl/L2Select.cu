...
__global__ void l2SelectMin1(
    ...
    // Each block handles kRowsPerBlock rows of the distances (results)
    Pair<T, unsigned short> threadMin[kRowsPerBlock];
    __shared__ Pair<T, unsigned short>
            blockMin[kRowsPerBlock * (kBlockSize / kWarpSize)];
...
#pragma unroll
    for (int i = 0; i < kRowsPerBlock; ++i) {
        threadMin[i].k = Limits<T>::getMax();
        threadMin[i].v = Limits<unsigned short>::getMax();
    }
    ...
    if (endRow) {
        for (int row = rowStart; row < productDistances.getSize(0); ++row) {
            for (int col = threadIdx.x; col < productDistances.getSize(1);
                ...
                if (Math<T>::lt(distance[0], threadMin[0].k)) {
                    threadMin[0].k = distance[0];
                    threadMin[0].v = (unsigned short)col;
                }
            }

            // Reduce within the block
            threadMin[0] = blockReduceAll<
                    Pair<T, unsigned short>,
                    Min<Pair<T, unsigned short>>,
                    false,
                    false>(
                    threadMin[0], Min<Pair<T, unsigned short>>(), blockMin);
            ...
            threadMin[0].k = Limits<T>::getMax();
            threadMin[0].v = Limits<unsigned short>::getMax();
        }
    } else {
        for (int col = threadIdx.x; col < productDistances.getSize(1);
             col += blockDim.x) {
...
#pragma unroll
        ...
        blockReduceAll<
                ...
                Min<Pair<T, unsigned short>>,
                false,
                false>(threadMin, Min<Pair<T, unsigned short>>(), blockMin);
...
__global__ void l2SelectMinK(
    ...
    __shared__ T smemK[kNumWarps * NumWarpQ];
    __shared__ unsigned short smemV[kNumWarps * NumWarpQ];
    ...
    BlockSelect<
            ...
            NumThreadQ,
            ThreadsPerBlock>
            heap(initK, Limits<unsigned short>::getMax(), smemK, smemV, k);
