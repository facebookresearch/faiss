...
void runDistance(
    ...
    DeviceTensor<unsigned short, 2, true> outIndexBuf1(
            res,
            makeTempAlloc(AllocType::Other, stream),
            {tileRows, numColTiles * k});
    DeviceTensor<unsigned short, 2, true> outIndexBuf2(
            res,
            makeTempAlloc(AllocType::Other, stream),
            {tileRows, numColTiles * k});
    DeviceTensor<unsigned short, 2, true>* outIndexBufs[2] = {
            &outIndexBuf1, &outIndexBuf2};
    ...
    for (idx_t i = 0; i < numQueries; i += tileRows) {
        ...
        for (idx_t j = 0; j < numCentroids; j += tileCols) {
            ...
            idx_t curCentroidSize = std::min(tileCols, numCentroids - j);
