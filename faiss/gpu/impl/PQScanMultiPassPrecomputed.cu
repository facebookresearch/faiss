...
// any number of sub-quantizers
template <typename EncodeT, int EncodeBits, typename CodeDistanceT>
__global__ void pqScanPrecomputedInterleaved( // (query id)(probe id)
        Tensor<float, 2, true> precompTerm1,
        // (centroid id)(codebook c)(sub q)(code id)
        Tensor<CodeDistanceT, 4, true> precompTerm2,
    ...
    ushort2 listId2 = topQueryToCentroid[queryId][probeId];
    // Safety guard in case NaNs in input cause no list ID to be generated
    if (listId2.x == Limits<ushort>::getMax() ||
        listId2.y == Limits<ushort>::getMax()) {
        return;
    }

    auto listId =
            toMultiIndex<ushort, int>(coarseCodebookSize, listId2.x, listId2.y);
...
__global__ void pqScanPrecomputedInterleaved( // (query id)(probe id)
        ...
        int coarseCodebookSize,
        Tensor<ushort2, 2, true> topQueryToCentroid,
        Tensor<EncodeT, 1, true, long> listCodes,
        int codeNumBytes,
    ...
    auto vecsBase = listCodes.data() + (long)listOffsets[listId] * codeNumBytes;
    int numVecs = listOffsets[listId + 1] - listOffsets[listId];
...
__global__ void pqScanPrecomputedMultiPass(
        Tensor<float, 2, true> precompTerm1,
        Tensor<LookupT, 4, true> precompTerm2,
    ...
    auto probeId = blockIdx.x;
    auto codesPerSubQuantizer = precompTerm2.getSize(3);
    auto precompTermSizePerCodebook =
            precompTerm2.getSize(2) * codesPerSubQuantizer;
    ...
    loadPrecomputedTerm<LookupT, LookupVecT>(
            term23,
            precompTerm2[listId2.x][0].data(),
            precompTerm3[queryId][0].data(),
            precompTermSizePerCodebook);

    loadPrecomputedTerm<LookupT, LookupVecT>(
            term23 + precompTermSizePerCodebook,
            precompTerm2[listId2.y][1].data(),
            precompTerm3[queryId][1].data(),
            precompTermSizePerCodebook);
...
__global__ void pqScanPrecomputedMultiPass(
    ...
    uint8_t* codeList =
            listCodes.data() + (long)listOffsets[listId] * codeNumBytes;
    int limit = listOffsets[listId + 1] - listOffsets[listId];
...
void runMultiPassTile(
        ...
        Tensor<idx_t, 2, true>& outIndices,
        cudaStream_t stream) {
    FAISS_ASSERT(precompTerm2.getSize(1) == 2);
    FAISS_ASSERT(precompTerm2.getSize(1) == precompTerm3.getSize(1));
    // Calculate offset lengths, so we know where to write out
    // intermediate results
    runCalcListOffsets(
            res,
...
void runPQScanMultiPassPrecomputed( // (query id)(probe id)
    ...
    for (int query = 0; query < topQueryToCentroid.getSize(0);
         query += queryTileSize) {
        int numQueriesInTile = std::min(
                queryTileSize, (int)topQueryToCentroid.getSize(0) - query);
...
void runPQScanMultiPassPrecomputed( // (query id)(probe id)
        ...
        Tensor<idx_t, 2, true>& outIndices,
        GpuResources* res) {
    runPQScanMultiPassPrecomputed<idx_t>(
            precompTerm1,
            precompTerm2,
            precompTerm3,
            coarseCodebookSize,
