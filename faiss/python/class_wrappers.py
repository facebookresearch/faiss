...
def handle_Index(the_class):
    ...
    def replacement_search(
        ...
        if numeric_type == faiss.Float32:
            ...
            self.search_ex(
                n, swig_ptr(x), numeric_type, k, swig_ptr(D), swig_ptr(I), params
            )
        return D, I
...
def handle_IndexBinary(the_class):
    ...
    def replacement_search(self, x, k, *, params=None):
        ...
        labels = np.empty((n, k), dtype=np.int64)
        self.search_c(n, swig_ptr(x), k, swig_ptr(distances), swig_ptr(labels), params)
...
def handle_Tensor2D(the_class):
    ...
    def replacement_init(self, *args):
        if len(args) == 1:
            ...
            faiss.copy_array_to_vector(np.ascontiguousarray(array).ravel(), self.v)
        else:
            self.original_init(*args)
...
def handle_Embedding(the_class):
    ...
    def from_array(self, array):
        ...
        faiss.copy_array_to_vector(np.ascontiguousarray(array).ravel(), self.weight)
