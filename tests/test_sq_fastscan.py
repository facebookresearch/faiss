...
class TestFastScanLUTScaleRounding(unittest.TestCase):
    """The uint16 accumulator boundary, which the span bound alone does not hold.

    round_tab computes floor(x * a + 0.5), so each of the M summed entries can
    gain up to 0.5 over its unrounded value. Bounding only the unrounded sum
    lets the rounded sum reach 65535 + M/2 and wrap. These cases sit on that
...
class TestRaBitQFastScanBiasBound(unittest.TestCase):
    ...
    def test_large_bias_spread_collapses(self):
        """Known limitation, NOT fixed here.

        Once the per-probe biases are far enough apart, RaBitQ fast scan gives
        R@1 0.03 against 1.00 for the non-fast-scan index. Bounding the sum by
        the bias does not help: it shrinks the scale until the quantized table
        loses its resolution, so the result is wrong either way. The bias
        probably has to leave the uint16 accumulator rather than be squeezed
        into it. Marked expected-failure so the day it starts passing is
        visible.
        """
        d, nlist, per, nq = 1024, 16, 40, 40
        rng = np.random.default_rng(7)
        centers = (np.arange(nlist, dtype=np.float32)[:, None] * 50.0) * np.ones(
            (1, d), dtype=np.float32
        )
        xb = np.repeat(centers, per, axis=0) + rng.standard_normal(
            (nlist * per, d)
        ...
        ref = recall_at_1(f"HR,IVF{nlist},RaBitQ4")
        fs = recall_at_1(f"HR,IVF{nlist},RaBitQfs4")
        self.assertGreaterEqual(fs, ref - 0.1)
