...
// Runtime-guarded (not #ifdef COMPILE_SIMD_AVX2, which is not defined for this
// test target) so it actually compiles and runs, mirroring
// RaBitQRearrangeBitPlanes.Avx2MatchesScalar above. AVX2 symbols always exist
// in the x86 faiss library (static and DD), so referencing them is safe here.
