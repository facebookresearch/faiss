...
class TestIndexIVFFlatPanorama(unittest.TestCase):
    ...
    def create_panorama(self, d, nlist, nlevels, xt, xb=None, nprobe=None, make_direct_map=False):
    ...
    def assert_range_results_equal(self, lims_regular, D_regular, I_regular, lims_panorama, D_panorama, I_panorama, nq):
        """Assert that range search results from two indices match."""
        np.testing.assert_array_equal(lims_regular, lims_panorama, err_msg="Different result counts")
        ...
        for i in range(nq):
            ...
            if n_results > 0:
                ...
                np.testing.assert_allclose(dist_reg[sort_reg], dist_pan[sort_pan], rtol=1e-5)
    ...
    def test_range_search(self):
        ...
        for radius in [0.5, 1.0, 2.0, 5.0]:
            with self.subTest(radius=radius):
                ...
                self.assertTrue(np.all(D_pan <= radius), f"Some distances exceed radius={radius}")
                self.assert_range_results_equal(lims_reg, D_reg, I_reg, lims_pan, D_pan, I_pan, nq)
    ...
    def test_id_selector_batch(self):
        ...
        np.testing.assert_array_equal(I_regular, I_panorama)
        np.testing.assert_allclose(D_regular[valid_panorama], D_panorama[valid_panorama], rtol=1e-5)
