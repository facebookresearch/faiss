...
void test_ivfpqfs_scanner(
        MetricType metric,
        IVFSearchParameters* params = nullptr,
    ...
    if (params != nullptr) {
        index_ivf->nprobe = params->nprobe;
    }
    auto xb = make_data(nb);
...
TEST(TestLowLevelIVF, IVFPQFS_by_residual_L2) {
    IVFSearchParameters params;
    params.nprobe = 1;
    test_ivfpqfs_scanner(METRIC_L2, &params, true);
}

TEST(TestLowLevelIVF, IVFPQFS_by_residual_IP) {
    IVFSearchParameters params;
    params.nprobe = 1;
    test_ivfpqfs_scanner(METRIC_INNER_PRODUCT, &params, true);
}
