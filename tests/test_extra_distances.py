...
class TestExtraDistances(unittest.TestCase):
    ...
    def test_L1(self):
        self.run_simple_dis_test(scipy.spatial.distance.cityblock, faiss.METRIC_L1)

    def test_Linf(self):
        self.run_simple_dis_test(scipy.spatial.distance.chebyshev, faiss.METRIC_Linf)
    ...
    def test_Lp(self):
        ...
        xq, yb = self.make_example()
        ref_dis = np.array(
            [[scipy.spatial.distance.minkowski(x, y, p) for y in yb] for x in xq]
        )
    ...
    def test_canberra(self):
        self.run_simple_dis_test(scipy.spatial.distance.canberra, faiss.METRIC_Canberra)
