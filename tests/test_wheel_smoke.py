...
class TestImportAndMetadata(unittest.TestCase):
    ...
    def test_compile_options(self):
        ...
        # In a DD build, compile options should mention DD
        # In a non-DD build, this still passes (just checks the function works)
...
class TestContribImports(unittest.TestCase):
    ...
    def test_contrib_torch_subpackage(self):
        ...
        import faiss.contrib.torch.quantization
...
class TestSIMD(unittest.TestCase):
    ...
    def test_simd_level_reported(self):
        # In a DD build, SIMDConfig should report available levels.
        # If DD is not compiled, this attribute may not exist — skip gracefully.
        if not hasattr(faiss, "get_compile_options"):
            raise unittest.SkipTest("get_compile_options not available")
        opts = faiss.get_compile_options()
